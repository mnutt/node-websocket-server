...
function Connection(server, req, socket, upgradeHead){
  if(server.options.log) sys.log("new connection: using "+(server.options.version));
  ...
  if(this._upgradeHead && this._upgradeHead.length == 8 && server.options.version == "draft76"){
    ...
    this.readyState = 2;
    if(server.options.log) sys.log("waiting.");
  }
};
...
Connection.prototype.write = function(data){
  ...
  if(this.readyState == 4){
    if(this._server.options.log) sys.log("write: "+this._id+" <"+data+">");
    if(socket.writable) {
      socket.write("\u0000", "binary");
    }
    if(socket.writable) {
...
Connection.prototype.close = function(){
  if(this._server.options.log) sys.log(this._id + ": closed");
...
function handshake(){
  ...
  this.readyState = 3;
  if(this._server.options.log) sys.log("handshake");
...
function handshakeWithDraft75(){
  if(this._server.options.log) sys.log("handshakeWithDraft75");
